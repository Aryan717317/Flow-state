...
- Default config auto-created at `~/.flow-state/config.json` on first run from `config.default.json`
...
- Removed `flow-state-switch` command (no longer needed — edit config directly)
...
- CLI commands: `flow-state`, `flow-state-server`, `flow-state-goal`
- Auto-start event server when running agent
- Centralized data directory at `~/.flow-state/`
- Proper Python package structure with `pyproject.toml`

### Changed
- Renamed `src/` to `flow_state/` for proper package naming
