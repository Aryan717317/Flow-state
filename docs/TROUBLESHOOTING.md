...
Lower it in `~/.flow-state/config.json`:
```json
...
1. Go to `chrome://extensions/` → verify **Flow State** is enabled
...
- `~/.flow-state/config.json` (remove API keys)
