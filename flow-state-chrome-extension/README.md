# Flow State Chrome Extension

Browser extension that tracks your activity and sends events to the Flow State server.
...
4. Select the `flow-state-extension` folder
...
2. Enable **Developer mode** (toggle in top-right)
3. Click **"Load unpacked"**
4. Select this directory (`flow-state-extension/`)
...
Make sure the Flow State server is running:
```bash
...
- You control all data through the Flow State server
